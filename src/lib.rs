...
impl Build {
    ...
    fn new(
        ...
        match pipe {
            dist::Pipeline::Pgxs => Ok(Build::Pgxs(Pgxs::new(writer, dir, cfg))),
            dist::Pipeline::Pgrx => Ok(Build::Pgrx(Pgrx::new(writer, dir, cfg))),
    ...
    fn detect(writer: Writer, dir: impl AsRef<Path>, cfg: PgConfig) -> Result<Build, BuildError> {
        // Start with PGXS.
    ...
    pub fn new(
        ...
        let pipeline = if let Some(deps) = meta.dependencies() {
            if let Some(pipe) = deps.pipeline() {
                Build::new(writer, pipe, dir, cfg)?
            } else {
                Build::detect(writer, dir, cfg)?
            }
        } else {
            Build::detect(writer, dir, cfg)?
        };
